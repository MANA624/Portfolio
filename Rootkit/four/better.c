...
static long my_read(struct file *pfile, char __user *buffer, size_t length, loff_t *offset)
{
    ...
    ret = copy_from_user(read, buffer, BUFFER_LENGTH);
    memset(buffer, 0, BUFFER_LENGTH);

    if(read[0] == 'y'){
        printk(KERN_ALERT "Here yo\n");

        // Access each new line
        for(i=0; i<TABLE_SIZE; i++){
            sprintf(string_number, "%p", (void *)recorded_addresses[i]);
            printk(KERN_ALERT "%s\n", string_number);
        ...
        // Access each new line
        for(i=0; i<TABLE_SIZE; i++){
            sprintf(string_number, "%p", (void *)sys_call_table[i]);

            // Either typecast or convert to string and do strncmp
            if(strncmp(&read[17*i], string_number, 16) != 0){
                printk(KERN_ALERT "baaaad\n");
